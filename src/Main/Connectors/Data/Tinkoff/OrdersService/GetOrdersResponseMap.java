...
public class GetOrdersResponseMap extends Mapping {
    protected OrderState[] orders;

    public GetOrdersResponseMap(BufferedReader bufferedReader) {
        boolean start = false;
        int index = 0; // to arrayCUN
        List<OrderState> ordersList = new ArrayList<>();
        ...
        try (BufferedReader br = bufferedReader) {
            ...
            while ((responseLine = br.readLine()) != null) {
                ...
                if (nowLine.equalsIgnoreCase("\"orders\": [{"))
                    start = true;
                ...
                if (start) {
                    ...
                    if (startId >= finalId) {
                        ...
                        ordersList.add(new OrderState(orderId, executionReportStatus, lotsRequested, lotsExecuted, arrayCUN[0], arrayCUN[1], arrayCUN[2], arrayCUN[4], arrayCUN[5], null, figi, direction, arrayCUN[6], orderType, null, null, instrumentUid, arrayCUN[3], stages, arrayCUN[7], orderCurrency, orderDate));
                        startId = 0;
            ...
            this.orders = new OrderState[ordersList.size()];
            ordersList.toArray(this.orders);
        } catch (Exception e) {
            System.out.println("Errors in \"GetOrderResponseMap\", line: " + ordersList.size() + "/" + startId + "\n" + e);
        }
    }
